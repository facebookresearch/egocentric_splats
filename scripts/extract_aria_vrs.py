...
def to_aria_image_frame(
    ...
    def process_raw_data(frame_i: int, camera_label: str):
        ...
        image_rel_path = (
            f"{camera_label}-images/{camera_label}_{capture_time_middle}.png"
        )
    ...
    for frame_i in tqdm(range(0, total_frames, num_process)):
        ...
        if img_frame is None:
            continue
        frames.append(to_nerfstudio_frame(img_frame, out_dir, visualize=visualize))
...
def to_nerfstudio_frame(
    ...
    if visualize:
        ...
        rr.log(f"world/device/{camera_name}_gain", rr.Scalar(frame.gain))
...
def visualize_frames(folder: Path, scale=100.0) -> None:
    ...
    for _, frame in enumerate(frames):
        ...
        rr.log(
            f"world/device/exposure_ms", rr.Scalar(frame["exposure_duration_s"] * 1000)
        )
        rr.log(f"world/device/gain", rr.Scalar(frame["gain"]))
...
def run_single_sequence(
    ...
    #     rectified_image_folder = output_path / f"{camera_label}-fisheye"

    #     for frame in frames:
    #         process_fisheye_frame(frame,  output_path, rectified_image_folder)
    ...
    for camera_label in camera_process_list:
        ...
        if camera_label == "camera-rgb":
            ...
            imageio.imwrite(rectified_image_folder / "vignette.png", vignette)
            ...
            imageio.imwrite(rectified_image_folder / "mask.png", mask)
            ...
            imageio.imwrite(rectified_image_folder / "image_index.png", image_index)
    ...
    # For RGB camera view, we use the points in nearest SLAM camera view that is also within the frustrum of RGB image.
...
def fetch_visible_depth_map_for_RGB(
    ...
    # find the nearest neighbour in the slam camera info
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert the VRS to json file format similar as nerfstudio",
    ...
    parser.add_argument(
        "--gaia_id", type=str, default="", help="Gaia ID of the recording folder"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eyegaze",
        help="path to 3D eye gaze csv file",
        type=str,
        default="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multi_sequence_json",
        help="Load multi sequence json files",
        type=str,
        default="",
    )
    parser.add_argument(
        "--simple_debayer",
        help="Apply simple debayering during image extraction",
        type=bool,
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=8080,
        help="aws cluster web port if used. (aws only)",
    )
    parser.add_argument(
        "--ws_port",
        type=int,
        default=9877,
        help="aws cluster ws port if used. (aws only)",
    )
    ...
    if args.gaia_id != "":
        recording_folder = args.input_root / f"gaia_{args.gaia_id}"
    else:
    ...
    if semidense_observation_path.exists() and args.semi_dense_observation_file != "":
        ...
        print(f"Will load semi-dense observatoin file: {semidense_observation_path}")
    else:
        print(
