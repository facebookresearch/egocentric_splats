...
def populate_render_tab(
    ...
    fov_degrees = server.add_gui_slider(
        "FOV",
        initial_value=113.68, # This is the FoV of ARIA videos
        min=0.1,
        max=175.0,
        step=0.01,
    ...
    with playback_folder:
        duration_number = server.add_gui_number("Duration (sec)", min=0.0, max=1e8, step=0.0001, initial_value=10.0)
    ...
    render_button = server.add_gui_button(
        ...
        hint="Generate the rendering command for rendering the camera path.",
    )
    ...
    def _(event: viser.GuiEvent) -> None:
        ...
        json_data["exposure"] = viewer.exposure
        json_data["gain"] = viewer.gain
        json_data["radiance_weight"] = viewer.radiance_weight                           
        json_data["gamma4sRGB"] = viewer.gamma
        json_data["apply_tonemapping"] = viewer.apply_tonemapping
