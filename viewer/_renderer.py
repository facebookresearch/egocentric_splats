"""
Refer to: 
https://github.com/hangg7/nerfview/blob/main/nerfview/_renderer.py
"""
...
class Renderer(threading.Thread):
    ...
    def _get_img_wh(self, aspect: float) -> Tuple[int, int]:
        max_img_res = self.viewer.max_image_res
        if self._state == "high":
            #  if True:
            H = max_img_res
            W = int(H * aspect)
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                traceback.print_exc()
                os._exit(1)
            #@todo: make jpg quality optional
            self.client.scene.set_background_image(
                img,
                format="jpeg",
                jpeg_quality=100 if task.action in ["static", "update"] else 70,
                depth=depth,
            )