...
from viewer import ViewerCustomized, populate_render_tab
from viewer.ui import TransformPanel, EditPanel

from scene.cameras import Camera, create_camera_from_JSON
...
class RenderViewer(ViewerCustomized):
    ...
    def init_from_root(self, model_root): 
        ...
        for p in model_paths:
            ...
            print(f"search for all model checkpoints under {p} that contain {self.point_cloud_file}")
        ...
        if len(valid_model_paths) < 1: 
            raise RuntimeError(f"Does not found valid model names to initialized from {model_root}")
    ...
    def init_from_model_path(self, model_path: Path, sub_path: str = None):
        ...
        self.viewer_renderer = load_from_model_path(cfg, model_path, ply_subpath=sub_path, bilateral_grid_path=bilateral_grid_path)
    ...
    def viewer_render_fn(self, camera_state, img_wh: Tuple[int, int]): 
        ...
        with torch.no_grad():
            render_pkg = self.viewer_renderer.render(camera, 
    ...
    def generate_guis(self):
        ...
        with tabs.add_tab("General"):
            ...
            if self.ply_names is not None:
                self.ply_drop_down = self.server.gui.add_dropdown(
                    "PLY files", tuple(self.ply_names),
                    initial_value=self.ply_names[-1]
                )
            ...
            def _(_) -> None:
                ...
                if self.ply_names is not None:
                    with self.server.atomic():
                        ...
                        self.ply_drop_down.options = scan_avail_iters(model_path, self.point_cloud_file)
                ...
                self.add_frame_slider_folder(self.real_cameras, slider_name="Observations")
                self.rerender(_)
        ...
        if self.enable_transform is True:
            with tabs.add_tab("Transform"):
                self.transform_panel = TransformPanel(self.server, self, self.loaded_model_count)
